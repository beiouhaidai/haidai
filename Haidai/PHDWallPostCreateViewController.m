//
//  PHDWallPostCreateViewController.m
//  Haidai
//
...
@interface PHDWallPostCreateViewController ()

@property (nonatomic, assign) NSUInteger maximumCharacterCount;

@end

@implementation PHDWallPostCreateViewController
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        _maximumCharacterCount = [[PHDConfigManager sharedManager] postMaxCharacterCount];
    }
    return self;
}
...
- (IBAction)postPost:(id)sender {
    ...
    PFObject *postObject = [PFObject objectWithClassName:PHDParsePostsClassName];
    postObject[PHDParsePostTextKey] = self.textView.text;
    postObject[PHDParsePostUserKey] = user;
    postObject[PHDParsePostLocationKey] = currentPoint;
    ...
    [postObject saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        ...
        if (succeeded) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:PHDPostCreatedNotification object:nil];
            });
        } else {
